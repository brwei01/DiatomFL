...
class DataAugmentationForPretrain(object):
    ...
    def __init__(self, args):
        ...
        if args.model_name == 'beit':
            ...
            if args.data_set == 'SSLpretrain_FL':
                self.common_transform = transforms.Compose([
                    #transforms.Resize(size=(args.input_size, args.input_size)),
                    transforms.RandomResizedCrop(args.input_size, scale=(0.7, 1.0), interpolation=InterpolationMode.BICUBIC),  # 3 is bicubic
                ...
                self.common_transform = transforms.Compose([
                    transforms.RandomResizedCrop(args.input_size, scale=(0.4, 1.0), interpolation=InterpolationMode.BICUBIC),  # 3 is bicubic
                    transforms.ColorJitter(hue=.05, saturation=.05),
                    transforms.RandomHorizontalFlip(p=0.5)])
            else:
                self.common_transform = transforms.Compose([
...
def build_transform(is_train, mode, args):
    ...
    if mode == 'finetune':
        if is_train:
            if args.data_set == 'datasets_diatom_FSSL':
